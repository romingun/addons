...
# 전기 누적량 최대 허용 증가량(kWh) lyh
POWER_TOTAL_MAX_DELTA = 10.0

# 마지막 정상 전기 누적량 저장 lyh
...
def ezville_loop(config):
    ...
    async def rev_meter_state(dev_sub_id, data):
        ...
        #lyh 전기총량 튀는 문제 해결
        global LAST_POWER_TOTAL
        
        await update_state('meter', 'current', rid, sbc, current_num)

        # 전기(03)만 이상값 검사
        if sbc == 3:
            current_total = float(total_num)
        
            if LAST_POWER_TOTAL is None:
                # 최초 수신은 저장
                LAST_POWER_TOTAL = current_total
                await update_state('meter', 'total', rid, sbc, total_num)
        
            else:
                delta = current_total - LAST_POWER_TOTAL
        
                # 정상 증가
                if 0 <= delta <= POWER_TOTAL_MAX_DELTA:
                    LAST_POWER_TOTAL = current_total
                    await update_state('meter', 'total', rid, sbc, total_num)
        
                else:
                    log('[INFO][SKIP] Electric meter ignored : prev {} >> new {}  delta {}'.format(LAST_POWER_TOTAL, current_total, delta))
       
        else:
            # 수도/가스/온수 등은 그대로 업데이트
            await update_state('meter', 'total', rid, sbc, total_num)
...
# asis                      await CMD_QUEUE.put({'sendcmd': sendcmd, 'recvcmd': recvcmd, 'statcmd': statcmd})
                            await CMD_QUEUE.put({'sendcmd': sendcmd, 'recvcmd': recvcmd, 'statcmd': statcmd, 'ew11no':'ew11_3'})
...
# asis                      log('[DEBUG] Queued ::: sendcmd: {}, recvcmd: {}, statcmd: {}'.format(sendcmd, recvcmd, statcmd))
                            log('[DEBUG] Queued ::: sendcmd: {}, recvcmd: {}, statcmd: {}, ew11no: {}'.format(sendcmd, recvcmd, statcmd, 'ew11_3'))
